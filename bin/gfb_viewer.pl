...
system('ls','/dev/shm') if ! -e $FB_FILE;
sleep 5;
# system(@cmd);
...
sub show_help {
    print <<"HELP";
Graphics::Framebuffer Viewer
This script sets up a shared memory framebuffer for viewing graphics output in real-time on X-Windows using ffplay or mplayer.
