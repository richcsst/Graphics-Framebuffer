...
sub vbox_flush {
    if ($F->{'VBOX'} and ($F->{'LAST_FLUSHED'} + $interval) <= time and ! $F->{'GFB_VIEWER'})) { # Only flush if in a real console, not the GUI
        $F->_flush_screen();
    }
}
...
Force a screen flush after each graphic operation.
